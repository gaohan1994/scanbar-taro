...
interface MemberAction {
  ...
  ) => Promise<ActionsInterface.ActionBase<any>>;
  /**
   * @interface memberAdd
   *
   * @memberof MemberAction
   ...
   */
  memberAdd: (params: {
    payload: MemberInterface.MemberInfoAddParams;
  }) => Promise<ActionsInterface.ActionBase<any>>;
...
class MemberAction {
  public getMemberLevelInfo = async (id: any) => {
    const result = await requestHttp.get(
      `/memberInfo/getMemberLevelInfo/${id}`
    );
    return result;
  };
      ...
      mergeData.forEach((item: any) => {
        ...
        if (dateIndex !== -1) {
          ...
          if (
            ...
            currentDateIndex.data.findIndex(
              d =>
                (d.id || d.businessNumber) === (item.id || item.businessNumber)
            ) !== -1
          ) {
            return;
          }
  ...
  public memberPreference = async (
    params: MemberInterface.MemberInfoDetail
  ) => {
