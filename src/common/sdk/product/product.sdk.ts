...
class ProductSDK {
  ...
  public getProductNumber = (
    ...
    const reduceCallback =
      this.sort !== this.reducerInterface.PAYLOAD_SORT.PAYLOAD_STOCK
        ? (prevTotal: number, item: ProductCartInterface.ProductCartInfo) =>
            prevTotal + item.sellNum
        : (prevTotal: number, item: ProductCartInterface.ProductCartInfo) =>
            prevTotal + (item.sellNum - item.number);
   ...
   */
  public getProductItemPriceType = (
  ...
  public getProductItemPrice = (
    ...
    if (!!currentMember && !!currentMember.enableMemberPrice) {
       ...
       */
      if (!!product.enableMemberDiscount) {
        const discountMemberPrice =
          product.price * currentMember.memberDiscount;
  ...
  public getCouponValue = (): number => {
    if (!!this.coupon) {
      return numeral(
        this.coupon && this.coupon.couponVO && this.coupon.couponVO.discount
      ).value();
    }
    return 0;
  };
  ...
  public getPointPrice = (
    ...
    for (let i = 0; i < productList.length; i++) {
      total +=
        this.getProductItemPrice(productList[i]) * productList[i].sellNum;
    }
  ...
  public getProductInterfacePayload = (
    ...
    const state = store.getState();
    const productList =
      products !== undefined ? products : state.productSDK.productCartList;
  ...
  public add = (
    ...
    sort:
      | ProductCartInterface.PAYLOAD_ORDER
      | ProductCartInterface.PAYLOAD_REFUND
      | ProductCartInterface.PAYLOAD_PURCHASE = this.reducerInterface
  ...
  public reduce = (
    ...
    sort:
      | ProductCartInterface.PAYLOAD_ORDER
      | ProductCartInterface.PAYLOAD_REFUND = this.reducerInterface.PAYLOAD_SORT
      .PAYLOAD_ORDER
  ) => {
    if (this.isWeighProduct(product)) {
      const reducer: ProductSDKReducer.ProductManageWeightCart = {
  ...
  public suspensionCart = async () => {
    ...
    if (state.productSDK.productCartList.length > 0) {
      const reducer: ProductSDKReducer.AddSuspensionCartPayload & {
        type: ProductCartInterface.ADD_SUSPENSION_CART;
      } = {
