...
class CartBar extends Taro.Component<CartBarProps, CartBarState> {
  ...
  public manageProduct = (
    ...
    if (
      type === productSdk.productCartManageType.ADD &&
      productSdk.isNonBarcodeProduct(product)
    ) {
    ...
    try {
      ...
      Taro.scanCode()
        .then(async res => {
          ...
          // 说明没找到
          const thirdProductResult = await ProductService.productInfoScan({
            barcode: res.result
          });
          Taro.hideLoading();
  ...
  public confirmChangeProduct = () => {
    ...
    productSdk.changeProduct(
      changeProduct as any,
      Number(changeSellNum),
      Number(changePrice),
      sort
    );
        ...
        >
          <View
            className={classnames(
              "cart-left-purchase-text",
              "cart-left-purchase-box",
              {
                [`cart-left-purchase-active`]: productCartList.length > 0
              }
            )}
          >
            <Text style={productCartList.length > 0 ? "color: #ffffff;" : ""}>
              {productCartList.length}
            </Text>
            <Text style={"color: #8c8c8c"}>种</Text>，
            <Text style={productCartList.length > 0 ? "color: #ffffff;" : ""}>
              {productSdk.getProductNumber(undefined)}
            </Text>
  ...
  private renderScan = () => {
    return (
      ...
      >
        <Image
          src="//net.huanmusic.com/weapp/icon_scan.png"
          className={`${cssPrefix}-scan-image`}
        />
      </View>
    );
  };
  ...
  private onWeightProductConfirm = () => {
    try {
      ...
      if (weightSellNum > 0) {
        let weightCartProduct: ProductCartInterface.ProductCartInfo = merge(
          {},
          changeWeightProduct
        ) as any;
        ...
        if (
          weightProductChangePrice !== "" &&
          numeral(weightProductChangePrice).value() !== weightCartProduct.price
        ) {
          weightCartProduct.changePrice = numeral(
            weightProductChangePrice
          ).value();
        }
  ...
  private renderWeightProductModal = () => {
    ...
    const weightInputs: ModalInput[] = [
      {
        ...
        onInput: ({ detail: { value } }) =>
          this.onChangeValue("weightProductSellNum", value),
        placeholder: "请输入重量"
      },
      {
        title: "价格",
        ...
        onInput: ({ detail: { value } }) =>
          this.onChangeValue("weightProductChangePrice", value),
        placeholder: "请输入商品价格"
      }
    ];
    const weightButtons = [
    ...
    const isOpen =
      changeWeightProduct &&
      typeof changeWeightProduct.id === "number" &&
      changeWeightProduct.id !== -1;
    return (
      <Modal
        isOpened={isOpen}
