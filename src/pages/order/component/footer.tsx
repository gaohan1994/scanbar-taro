...
class OrderFooter extends Taro.Component<Props> {
  render() {
    ...
    return (
      <View className={`${prefix}-footer`} style="background-color: #fff">
        <View className={`${prefix}-footer-normal`}>
          <Text className={`${prefix}-footer-bge`}>￥</Text>
          <Text className={`${prefix}-footer-price`}>
            {price.split(".")[0]}
          </Text>
          <Text className={`${prefix}-price-bge ${prefix}-price-pos`}>{`.${
            price.split(".")[1]
          }`}</Text>
        </View>
        <View className={`${prefix}-footer-button`} onClick={onClick}>
          退货退款
        </View>
      </View>
    );
