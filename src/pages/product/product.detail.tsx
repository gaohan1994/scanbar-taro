...
class ProductDetail extends Taro.Component<Props, State> {
  ...
  public fetchProductDetail = async (id: string) => {
    try {
      ...
      const productDetail: ProductInterface.ProductInfo = merge(
        {},
        result.data
      );
      ...
      this.setState({
        typePickerValue: productType.findIndex(
          t => t.id === productDetail.typeId
        ),
        supplierValue: productSupplier.findIndex(
          s => s.id === productDetail.supplierId
        )
      });
    } catch (error) {
      Taro.showToast({
      ...
      .then(async scanResult => {
        try{
          ...
          if (thirdResult.code === ResponseCode.success) {
            ...
            const newProductChangeDetail: Partial<ProductInterface.ProductInfo> = merge(
              {},
              productChangeDetail
            );
  ...
  render() {
    const { productDetail, productChangeDetail, typePickerValue, isEdit } = this.state;
    ...
    const formNumber: FormRowProps[] = [
      {
        title: "库存",
        extraText: `${numeral(
          productChangeDetail.number || productDetail.number
        ).value()}`,
      ...
      },
      {
        title: "库存下限预警",
        extraText: `${numeral(
          productChangeDetail.limitNum || productDetail.limitNum
        ).value()}`,
    ...
    const formPrice: FormRowProps[] = [
      {
        title: "进价",
        extraText: `￥${numeral(
          productChangeDetail.cost || productDetail.cost
        ).format("0.00")}`,
      ...
      },
      {
        title: "会员价",
        extraText: `${
          !!productChangeDetail.memberPrice
      ...
      return (
        <View className="container">
          ...
          <View
            ...
            className={`${cssPrefix}-detail-list  product-detail product-add`}
          >
            ...
            >
              ...
              >
                <FormRow
                  title="类别"
                  extraText={
                    productChangeDetail.typeName || productDetail.typeName
                  }
                  arrow="right"
                />
              </Picker>
    ...
    const costInputs: ModalInput[] = [
      {
        ...
        onInput: ({ detail: { value } }) =>
          this.onNumberValueChange("cost", value)
      },
      {
        title: "进价差额",
        value: `${
          !!productChangeDetail.cost && String(productChangeDetail.cost) !== ""
            ? numeral(
                numeral(productChangeDetail.cost).value() -
                  numeral(productDetail.cost).value()
              ).format("0.00")
            : ""
        }`,
        disabled: true
    ...
    const priceInputs: ModalInput[] = [
      {
        ...
        onInput: ({ detail: { value } }) =>
          this.onNumberValueChange("price", value)
      },
      {
        title: "售价差额",
        value: `${
          !!productChangeDetail.price &&
          String(productChangeDetail.price) !== ""
            ? numeral(
                numeral(productChangeDetail.price).value() -
                  numeral(productDetail.price).value()
              ).format("0.00")
            : ""
        }`,
        disabled: true
    ...
    const memberPriceInputs: ModalInput[] = [
      {
        title: "会员价",
        type: "digit",
        placeholder: `${typeof productDetail.memberPrice === 'undefined' ? "" :  productDetail.memberPrice}`,
        value: `${productChangeDetail.memberPrice || ""}`,
        onInput: ({ detail: { value } }) =>
          this.onNumberValueChange("memberPrice", value)
        // focus: true,
      },
      {
        title: "价格差额",
        value: `${
          !!productChangeDetail.memberPrice &&
          String(productChangeDetail.memberPrice) !== ""
            ? numeral(
                numeral(productChangeDetail.memberPrice).value() -
                  numeral(typeof productDetail.memberPrice === 'undefined' ? productDetail.price : productDetail.memberPrice).value()
              ).format("0.00")
            : ""
        }`,
        disabled: true
    ...
    const numberInputs: ModalInput[] = [
      {
        ...
        placeholder: "请输入现有库存",
        onInput: ({ detail: { value } }) =>
          this.onNumberValueChange("number", value)
        // focus: true,
      },
      {
        title: "库存差额",
        value: `${
          !!productChangeDetail.number &&
          String(productChangeDetail.number) !== ""
            ? numeral(
                numeral(productChangeDetail.number).value() -
                  numeral(productDetail.number).value()
              ).format("0.00")
            : ""
        }`,
        disabled: true
    ...
    const numberLimitInputs: ModalInput[] = [
      {
        ...
        onInput: ({ detail: { value } }) =>
          this.onNumberValueChange("limitNum", value)
        // focus: true,
      },
      {
        title: "预警差额",
        value: `${
          !!productChangeDetail.limitNum &&
          String(productChangeDetail.limitNum) !== ""
            ? numeral(
                numeral(productChangeDetail.limitNum).value() -
                  numeral(productDetail.limitNum).value()
              ).format("0.00")
            : ""
        }`,
        disabled: true
    ...
    return (
      <View>
        ...
        <Modal
          ...
          header="会员价调整"
          tip={`系统会员价：${typeof productDetail.memberPrice === 'undefined' ? productDetail.price : productDetail.memberPrice}`}
          onClose={() =>
            this.changeModalVisible("memberPriceModalVisible", false)
          }
