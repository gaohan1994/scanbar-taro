 ...
 * @Last Modified time: 2020-04-28 16:58:19
 *
 */
...
class ProductSuspension extends Taro.Component<Props, State> {
  ...
  public componentDidShow() {
    ...
    if (suspensionCartList.length > 0) {
      this.setState({
        currentSuspension: suspensionCartList[0].suspension.date
      });
    }
  }

  public onSuspensionClick = (
    suspension: ProductSDKReducer.SuspensionCartBase
  ) => {
        ...
        if (result.confirm) {
          ...
          if (suspension) {
            ...
            if (nextSuspensionCartList.length > 0) {
              this.setState({
                currentSuspension: nextSuspensionCartList[0].suspension.date
              });
              return;
            }
          }
    ...
    return (
      <View className={`container ${cssPrefix}`}>
        {suspensionCartList && suspensionCartList.length > 0 ? (
          <View
            className={`${cssPrefix}-list-container ${cssPrefix}-list-container-suspension`}
          >
            {this.renderLeft()}
            {this.renderRight()}
          </View>
          ...
          suspensionCartList.map(suspension => {
            return (
              ...
              >
                ...
                <Text>
                  {dayJs(suspension.suspension.date).format("MM月DD日")}
                </Text>
  ...
  private renderRight = () => {
    ...
    const currentSuspensionList =
      suspensionCartList.find(s => s.suspension.date === currentSuspension) ||
      {};
    const {
      productCartList
    } = currentSuspensionList as ProductSDKReducer.SuspensionCartBase;
    return (
                ...
                >
                  <View className={`${cssPrefixComponent}-content`}>
                    <View className={`${cssPrefixComponent}-content-cover`}>
                      {product.pictures && product.pictures !== "" ? (
                        ...
                        <Image
                          ...
                          className={`${cssPrefixComponent}-content-cover-image`}
                        />
                      )}
                    </View>
                    <View className={`${cssPrefixComponent}-content-detail`}>
                      <View className={`${cssPrefixComponent}-title`}>
                        {product.name}
                      </View>
                      <Text className={`${cssPrefixComponent}-normal`}>
                        <Text className={`${cssPrefixComponent}-price-bge`}>
                          ￥
                        </Text>
                        <Text className={`${cssPrefixComponent}-price`}>
                          {product.price}
                        </Text>
                        /{product.unit}
                      </Text>
                    ...
                    <View className={`${cssPrefixComponent}-stepper`}>
                      <View
                        className={`${cssPrefixComponent}-stepper-container`}
                      >
                        <View
                          className={classnames(
                            `${cssPrefixComponent}-stepper-button`,
                            `${cssPrefixComponent}-stepper-button-reduce`
                          )}
                          onClick={() =>
                            this.manageProduct(
                              product,
                        ...
                        />
                        <Text className={`${cssPrefixComponent}-stepper-text`}>
                          {product.sellNum}
                        </Text>
                        {!productSdk.isWeighProduct(product) && (
                          <View
                            className={classnames(
                              `${cssPrefixComponent}-stepper-button`,
                              `${cssPrefixComponent}-stepper-button-add`
                            )}
                            onClick={() =>
                              this.manageProduct(
                                product,
